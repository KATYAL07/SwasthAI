/**
 * Postgres data layer (Supabase) — the only data store.
 *
 * Exposes dbRun / dbGet / dbAll / dbExec taking `?`-style SQL, so the call sites
 * in server.ts stay free of dialect details. Three translations make that work:
 *
 *   1. Placeholders.  The call sites write `?`; Postgres takes `$1, $2`.
 *      Converted here, skipping anything inside a string literal so a `?` in
 *      text survives.
 *
 *   2. Identifier case.  Postgres folds unquoted identifiers to lower case, so
 *      `doctorId` is stored as `doctorid` and comes back that way. The queries
 *      still match (both sides fold), but `row.doctorId` would silently be
 *      undefined everywhere. Result rows are mapped back to camelCase below.
 *
...
export function createPostgresBackend(connectionString: string): DatabaseBackend {
  ...
  try {
    ...
    location = `postgres://${u.hostname}${u.port ? ":" + u.port : ""}${u.pathname}`;
  ...
  async function dbRun(sql: string, params: any[] = []) {
    ...
    // shape still matches what the DatabaseBackend contract promises.
    return { lastID: 0, changes: res.rowCount ?? 0 };
  }

  async function dbGet<T = any>(sql: string, params: any[] = []): Promise<T | null> {
    const res = await pool.query(toPositional(sql), params);
   ...
   * Supabase SQL editor — it creates triggers and row-level-security policies
   * that need to be reviewed, not silently re-run by an application boot.
   *
   * Only the three reference tables are seeded. Sample appointments, queue
   * tokens and medical records are not: those rows carry a patientId that must
   * reference a real Supabase auth account, and none exists on a fresh project.
   * They appear once a signed-in user creates them.
   */
  async function initializeDatabase() {
    console.log(`[Postgres] Verifying schema at ${location}...`);
