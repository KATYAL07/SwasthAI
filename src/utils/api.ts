/**
 * Client API Helpers for SwasthAI Full-Stack Integration
 */
...
export const SESSION_EXPIRED_EVENT = "swasthai:session-expired";
...
export async function apiFetch<T>(url: string, options?: RequestInit, retries = 3, delayMs = 300): Promise<T> {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // Only transient network failures are retried. There is no local fallback:
      // Supabase is the single source of data, and a refusal is an answer.
